...
class ToolbarWidget extends StatefulWidget {
  ...
  final Function? funLoading, funError;
...
class ToolbarWidgetState extends State<ToolbarWidget> {
  ...
  bool _isLock = false;
  String url_hainong = '';
  Future gethainong() async{
    final prefs = await SharedPreferences.getInstance();
    url_hainong =  prefs.getString('hainong_url')!;
  }
          ...
          onPressed: (int index) async {
            ...
            if (t.getIcons()[index].icon == Icons.image_outlined) {
              ...
              if (proceed) {
                ...
                if (result != null) {
                  if (result.files.first.size < 102400000) {
                    widget.funLoading!();
                    var image = await sentImage(result);
                    Navigator.of(context).pop();
                    if (image.isNotEmpty) {
                      var proceed = await widget.htmlToolbarOptions.mediaLinkInsertInterceptor?.call(image, InsertFileType.image) ?? true;
                      if (proceed) widget.controller.insertNetworkImage(image);
                    } else widget.funError!();
                  } else widget.funError!(error: 'Kích thước ảnh quá lớn (Kích thước phải dưới 100Mb)');
                }
              }
              _isLock = false;
              return;
            ...
            if (t.getIcons()[index].icon == Icons.videocam_outlined) {
              ...
              if (proceed) {
                ...
                if (result != null) {
                  if (result.files.first.size < 102400000) {
                    widget.funLoading!();
                    var video = await sentImage(result, isVideo: true);
                    Navigator.of(context).pop();
                    if (video.isNotEmpty) {
                      var proceed = await widget.htmlToolbarOptions.mediaLinkInsertInterceptor?.call(video, InsertFileType.video) ?? true;
                      if (proceed) {
                        widget.controller.insertHtml("<video style='max-width: 100%; max-height: 100%; width:device-width; height:auto' controls='' src='${video.toString()}#t=0.1'></video><br>");
                      }
                    } else widget.funError!();
                  } else widget.funError!(error: 'Kích thước video quá lớn (Kích thước phải dưới 100Mb)');
                }
              }
              _isLock = false;
              return;
  ...
  Future sentImage(FilePickerResult file, {bool isVideo = false}) async {
    var request = http.MultipartRequest('POST', Uri.parse(url_hainong + '/api/v2/base/upload_file'));
    request.files.add(await http.MultipartFile.fromPath('file', file.files.single.path!));
    try {
      var res = await request.send().timeout(Duration(minutes: isVideo ? 10 : 1));
      if (res.statusCode == 200) {
        var response = await res.stream.toBytes();
        var result = String.fromCharCodes(response);
        final json = jsonDecode(result);
        return _checkJsonKey(json, 'success') && json['success'] && _checkJsonKey(json, 'data') ? json['data'].toString() : '';
      }
    } catch (_) {}
    return '';
  }

  Future<String> _fetchData(BuildContext context, FilePickerResult? result) async {
   final String url;
    showDialog(
  ...
  bool _checkJsonKey(json, key) => json.containsKey(key) && json[key] != null && json.isNotEmpty;
}
